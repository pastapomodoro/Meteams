...
private func resolvePython() -> String {
    pythonPaths.first { FileManager.default.fileExists(atPath: $0) } ?? "/usr/bin/python3"
}

private let pidFile = NSHomeDirectory() + "/.meteo_jiggle.pid"

private let jiggleScript = """
import ctypes,os,sys,signal,time,subprocess
PID_FILE=sys.argv[1]
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func startDaemon() {
        let p = Process()
        p.executableURL = URL(fileURLWithPath: resolvePython())
